...
def main():
    ...
    logging.debug(dumps(args, indent=4))
    ...
    if args['command'] == 'single':
        ...
        try:
            with open(args['config-file'][0], 'r') as f:
                ...
                try:
                    ...
                    logging.critical('File {}, line {}: {}'.format(args['config-file'][0], reader.line_num, e))
                    exit(1)
        except:
            logging.critical('Invalid command line argument: {} does not exist'.format(args['config-file'][0]))
            exit(1)

    logging.debug('Collection-specific parameters:')
    logging.debug(dumps(collections, indent=4))

    if len(collections) == 0:
        logging.error('no collections to process')
        exit(1)
        ...
        for collection in collections:
            try:
                ...
                logging.error('Unable to generate "{}" for collection "{}"'.format(
                    'resourcelist' if collection['strategy'] == 0 else
                    'new_changelist' if collection['strategy'] == 1 else
                    'inc_changelist',
